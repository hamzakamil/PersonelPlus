    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      <div class="p-6">
        ...
        <div v-for="workplace in activeWorkplaces" :key="workplace._id" class="mb-6">
          ...
          <div
            class="flex items-center justify-between p-3 bg-green-50 rounded-lg mb-3 border-2 border-green-200"
          >
            <div class="flex items-center flex-wrap gap-2">
              <span class="text-sm font-semibold text-gray-800">{{ workplace.name }}</span>
              <span v-if="workplace.isDefault" class="text-xs text-gray-600"
                >(Varsayılan SGK İşyeri)</span
              >
              <span
                v-if="workplace.sgkRegisterNumber"
                class="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded"
              >
                SGK: {{ workplace.sgkRegisterNumber }}
              </span>
              <span
                v-if="workplace.manager"
            ...
            <div class="flex gap-1">
              <span class="text-xs text-gray-500 mr-2"
                >{{ getWorkplaceDepartments(workplace._id).length }} departman</span
              >
            </div>
          </div>
            ...
            >
              ...
              >
                <div class="flex items-center flex-wrap gap-2">
                  ...
                  <span class="text-xs bg-blue-200 text-blue-800 px-2 py-0.5 rounded"
                    >Departman</span
                  >
                  <span v-if="dept.description" class="text-sm text-gray-500"
                    >({{ dept.description }})</span
                  >
                  <span
                    v-if="dept.workingHours"
              ...
              <div v-if="getDepartmentSections(dept._id).length > 0" class="ml-6 mt-2 space-y-2">
                <div v-for="section in getDepartmentSections(dept._id)" :key="section._id">
                  <div
                    class="flex items-center justify-between p-2 bg-purple-50 rounded-lg border border-purple-200"
                  >
                    <div class="flex items-center flex-wrap gap-2">
                      ...
                      <span class="text-xs bg-purple-200 text-purple-800 px-2 py-0.5 rounded"
                        >Bölüm</span
                      >
                      <span v-if="section.description" class="text-sm text-gray-500"
                        >({{ section.description }})</span
                      >
                      <span
                        v-if="section.workingHours"
                        class="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded"
                      >
                    ...
                    <div class="flex gap-1">
                      ...
                      <button
                        @click="deleteDepartment(section._id)"
                        class="p-1.5 text-red-600 hover:bg-red-50 rounded"
                        title="Sil"
                      >
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div v-if="getSectionUnits(section._id).length > 0" class="ml-6 mt-2 space-y-2">
                    ...
                    >
                      <div class="flex items-center flex-wrap gap-2">
                        ...
                        <span class="text-xs bg-yellow-200 text-yellow-800 px-2 py-0.5 rounded"
                          >Birim</span
                        >
                        <span v-if="unit.description" class="text-sm text-gray-500"
                          >({{ unit.description }})</span
                        >
                        <span
                          v-if="unit.workingHours"
                          class="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded"
                        >
                      ...
                      <div class="flex gap-1">
                        ...
                        <button
                          @click="deleteDepartment(unit._id)"
                          class="p-1.5 text-red-600 hover:bg-red-50 rounded"
                          title="Sil"
                        >
                          <svg
        ...
        <div v-if="unassignedDepartments.length > 0" class="mb-6">
          <div
            class="flex items-center justify-between p-3 bg-gray-100 rounded-lg mb-3 border-2 border-gray-300"
          >
            <div class="flex items-center flex-wrap gap-2">
              <span class="text-sm font-semibold text-gray-700">İşyeri Atanmamış</span>
              <span class="text-xs text-gray-500"
                >(Bu departmanlar bir SGK işyerine bağlı değil)</span
              >
            </div>
          </div>
        ...
        >
          ...
          <div class="space-y-2">
            <div v-for="dept in filteredInactiveDepartments" :key="dept._id">
              <!-- Ana pasif departman -->
              <div
              ...
              >
                ...
                <div class="flex gap-1">
                  <button
                    @click="activateDepartment(dept._id)"
              ...
              <div v-if="getInactiveChildren(dept._id).length > 0" class="ml-12 mt-2 space-y-2">
                ...
                >
                  <div class="flex items-center flex-wrap gap-2">
                    ...
                    <span v-if="child.description" class="text-sm text-gray-400"
                      >({{ child.description }})</span
                    >
                    <span class="text-xs bg-gray-300 text-gray-700 px-2 py-0.5 rounded"
                      >Pasif Alt Kayıt</span
                    >
                  </div>
                  <div class="flex gap-1">
                    <button
                      @click="activateDepartment(child._id)"
                      class="p-1.5 text-green-600 hover:bg-green-50 rounded"
                      title="Aktif Et"
                    >
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                      @click="deleteDepartment(child._id)"
                      class="p-1.5 text-red-600 hover:bg-red-50 rounded"
                      title="Sil"
                    >
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="p-4 bg-gray-50 border-t">
        <div class="flex justify-between items-center">
          <p class="text-sm text-gray-600">Toplam {{ totalCount }} kayıt</p>
    ...
    >
      <div class="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form @submit.prevent="saveDepartment">
          <div class="space-y-4">
            <Input v-model="form.name" label="Ad" required uppercase />
            ...
            <div>
              ...
              >
                ...
                <option v-for="wp in activeWorkplaces" :key="wp._id" :value="wp._id">
                  {{ wp.name }} {{ wp.isDefault ? '(Varsayılan)' : '' }}
                </option>
              </select>
              <p class="text-xs text-gray-500 mt-1">Departmanın bağlı olduğu SGK işyerini seçin</p>
            </div>

            <!-- Üst Seviye (Departman/Bölüm) seçimi -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1"
                >Üst Seviye (Opsiyonel)</label
              >
              <select
                v-model="form.parent"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                ...
                <optgroup
                  v-if="getAvailableParentDepartments.length > 0"
                  label="Departmanlar (Bölüm olarak ekle)"
                >
                  <option
                ...
                <optgroup
                  v-if="getAvailableSections.length > 0"
                  label="Bölümler (Birim olarak ekle)"
                >
                  <option
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1"
                >Mesai Saatleri (Opsiyonel)</label
              >
              <select
              ...
              >
                ...
                <option v-for="wh in availableWorkingHours" :key="wh._id" :value="wh._id">
                  {{ wh.name }}
                </option>
              </select>
            </div>
            <div>
              ...
              >
                ...
                <option v-for="emp in availableEmployees" :key="emp._id" :value="emp._id">
                  {{ emp.firstName }} {{ emp.lastName }}
                  {{ emp.position ? `(${emp.position})` : '' }}
                </option>
              </select>
              <p class="text-xs text-gray-500 mt-1">İzin talepleri bu yöneticiye iletilecektir.</p>
            </div>
...
const getHierarchyLabel = dept => {
  ...
  const parentId =
    dept.parentDepartment?._id || dept.parentDepartment || dept.parent?._id || dept.parent;
...
const activeParentDepartments = computed(() => {
  ...
  return departments.value.filter(
    dept => dept.isActive === true && !dept.parentDepartment && !dept.parent
  );
});
...
const allSections = computed(() => {
  ...
  return departments.value.filter(dept => {
    ...
    if (!parent) return false;
    const parentIdValue =
      typeof parent === 'object' && parent._id ? parent._id.toString() : parent?.toString();
...
const getAvailableSections = computed(() => {
  ...
  const parentDeptIds = new Set(getAvailableParentDepartments.value.map(d => d._id.toString()));
  ...
  return departments.value.filter(
    dept =>
      ...
      (!dept.parentDepartment ||
        (typeof dept.parentDepartment === 'object' && !dept.parentDepartment._id))
  );
});
...
const availableParents = computed(() => {
  if (editingDepartment.value) {
    return departments.value.filter(dept => {
      const parent = dept.parentDepartment || dept.parent;
      return (
        dept._id !== editingDepartment.value._id &&
        (!parent ||
          (typeof parent === 'object' ? parent._id : parent) !== editingDepartment.value._id)
      );
    });
  }
...
const matchesSearch = dept => {
  ...
  return (
    dept.name?.toLowerCase().includes(search) || dept.description?.toLowerCase().includes(search)
  );
};
