<template>
  <div class="p-6">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(request, index) in sortedRequests" :key="request._id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              <span v-if="request.currentApprover && (request.status === 'PENDING' || request.status === 'IN_PROGRESS')">
    ...
    <div v-if="showCancellationModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <div v-if="cancellationRequest" class="mb-4 p-3 bg-gray-50 rounded-lg">
          <p class="text-sm text-gray-600">
            <strong>İzin:</strong> {{ cancellationRequest.leaveSubType?.name || cancellationRequest.companyLeaveType?.name || cancellationRequest.type }}
          </p>
          <p class="text-sm text-gray-600">
            <strong>Tarih:</strong> {{ formatDate(cancellationRequest.startDate) }} - {{ formatDate(cancellationRequest.endDate) }}
          </p>
        </div>
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
          <p class="text-sm text-yellow-800">
            <strong>Not:</strong> İptal talebiniz, izninizi onaylayan yöneticiler tarafından değerlendirilecektir.
          </p>
        </div>
    ...
    <div v-if="showDetailModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 w-full max-w-2xl">
        ...
        <div class="space-y-4" v-if="selectedRequest">
          ...
          <div v-if="selectedRequest.status === 'CANCELLATION_REQUESTED' && selectedRequest.cancellationReason" class="bg-orange-50 border border-orange-200 rounded-lg p-3">
          ...
          <div v-if="selectedRequest.currentApprover && (selectedRequest.status === 'PENDING' || selectedRequest.status === 'IN_PROGRESS')">
            <strong>Onay Bekleniyor:</strong> {{ selectedRequest.currentApprover?.firstName }} {{ selectedRequest.currentApprover?.lastName }}
          </div>
          <div v-if="selectedRequest.cancellationCurrentApprover && selectedRequest.status === 'CANCELLATION_REQUESTED'">
            <strong>İptal Onayı Bekleniyor:</strong> {{ selectedRequest.cancellationCurrentApprover?.firstName }} {{ selectedRequest.cancellationCurrentApprover?.lastName }}
          </div>
