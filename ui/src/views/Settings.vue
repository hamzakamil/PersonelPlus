  ...
  <div class="p-4">
    ...
    <div v-if="showPasswordChangeModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-4 w-full max-w-sm">
        ...
        <p class="text-xs text-gray-600 mb-3">İlk girişinizde şifrenizi değiştirmeniz gerekmektedir.</p>
        <form @submit.prevent="changePassword">
          <div class="space-y-3">
            <Input v-model="passwordForm.newPassword" type="password" label="Yeni Şifre" required minlength="6" />
            <Input v-model="passwordForm.confirmPassword" type="password" label="Yeni Şifre Tekrar" required />
            ...
            <div class="flex gap-2 justify-end">
              <Button type="submit" size="sm" :disabled="passwordForm.newPassword !== passwordForm.confirmPassword || passwordForm.newPassword.length < 6">
                Şifreyi Değiştir
              </Button>
            </div>
          </div>
    ...
    <!-- Sekmeler -->
    <div class="mb-4 border-b border-gray-200 bg-gradient-to-r from-slate-50 to-gray-50 rounded-t-lg">
      ...
      <form @submit.prevent="saveDealerSettings" class="space-y-6">
        <div class="space-y-4">
          ...
          <p class="text-sm text-gray-500">
            İşe giriş/çıkış ekranlarında şirketlere görünecek isim. Boş bırakılırsa bayi adı kullanılır.
          </p>
    ...
    <div v-show="activeTab === 'company'" class="bg-white rounded-lg shadow p-6">
      <h3 class="text-lg font-semibold mb-6 text-gray-800 border-b pb-3">İşletme / Şirket Bilgileri</h3>
      <form @submit.prevent="saveCompanyInfo" class="space-y-6">
        ...
        <div class="flex justify-end pt-4 border-t">
          <Button type="submit" size="md" :disabled="savingCompany">
            <svg v-if="savingCompany" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <form @submit.prevent="saveSettings" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select v-model="form.payrollCalculationType" @change="savePayrollType" class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          ...
          <input type="file" accept="image/jpeg,image/png,image/gif" @change="handleFileChange" class="block w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
          <p class="text-[10px] text-gray-500 mt-1">Maksimum 500KB, önerilen boyut: 200x50 piksel (JPEG, PNG, GIF)</p>
        ...
        <div>
          ...
          <select v-model="form.activeAttendanceTemplate" @change="saveTemplateSetting" class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="">Şablon Seçiniz</option>
            <option v-for="template in attendanceTemplates" :key="template._id" :value="template._id">
        ...
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
          <div>
            ...
            <p class="text-[10px] text-gray-500">Onaylanan işe giriş talepleri otomatik olarak çalışanlar listesine eklensin</p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" v-model="form.autoAddApprovedEmployees" @change="saveAutoAddSetting" class="sr-only peer" />
            <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        </div>

        <div class="flex justify-end">
          <Button type="submit" size="sm" :disabled="loading">{{ loading ? 'Kaydediliyor...' : 'Kaydet' }}</Button>
        </div>
      </form>
    </div>
    ...
    <div v-show="activeTab === 'checkin'" class="bg-white rounded-lg shadow p-4">
      ...
      <div class="space-y-4">
        <label class="flex items-center">
          <input type="checkbox" v-model="form.checkInSettings.enabled" class="mr-2 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
        ...
        <div v-if="form.checkInSettings.enabled" class="space-y-3 pl-4 border-l-2 border-gray-200">
          <label class="flex items-center">
            <input type="checkbox" v-model="form.checkInSettings.locationRequired" class="mr-2 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
          ...
          <div v-if="form.checkInSettings.locationRequired" class="space-y-3">
            <div class="grid grid-cols-3 gap-3">
              <Input v-model.number="form.checkInSettings.allowedLocation.latitude" type="number" step="0.000001" label="Enlem" />
              <Input v-model.number="form.checkInSettings.allowedLocation.longitude" type="number" step="0.000001" label="Boylam" />
              <Input v-model.number="form.checkInSettings.allowedLocation.radius" type="number" label="Yarıçap (m)" />
            </div>
            <Button type="button" variant="secondary" size="sm" @click="getCurrentLocation" :disabled="gettingLocation">
              {{ gettingLocation ? 'Konum alınıyor...' : 'Mevcut Konumu Kullan' }}
            </Button>
          </div>

          <label class="flex items-center">
            <input type="checkbox" v-model="form.checkInSettings.autoCheckIn" class="mr-2 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
    ...
    <div v-show="activeTab === 'weekend'" class="bg-white rounded-lg shadow p-4">
      ...
      <div class="flex gap-2 mb-4">
        <button @click="weekendTab = 'company'" :class="weekendTab === 'company' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'" class="px-3 py-1 text-xs rounded">Şirket</button>
        <button @click="weekendTab = 'department'" :class="weekendTab === 'department' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'" class="px-3 py-1 text-xs rounded">Departman</button>
        <button @click="weekendTab = 'employee'" :class="weekendTab === 'employee' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'" class="px-3 py-1 text-xs rounded">Çalışan</button>
      </div>
      ...
      <div v-if="weekendTab === 'company'" class="space-y-4">
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <label v-for="day in weekDays" :key="day.value" class="flex items-center px-3 py-1.5 border rounded cursor-pointer hover:bg-gray-50 text-xs"
              :class="companyWeekendDays.includes(day.value) ? 'bg-blue-100 border-blue-500' : 'bg-white border-gray-300'">
              <input type="checkbox" :value="day.value" v-model="companyWeekendDays" class="mr-2 w-3 h-3" @change="saveWeekendSettings" />
              {{ day.label }}
            </label>
          </div>
        </div>
        ...
        <div v-if="companyWeekendDays.length >= 2" class="border-t pt-4">
          <label class="block text-xs font-medium text-gray-700 mb-2">Yıllık İzin Hesaplamasında Hafta Tatili</label>
          <div class="space-y-2">
            <label v-for="option in leaveDeductionOptions" :key="option.value" class="flex items-center p-2 border rounded cursor-pointer hover:bg-gray-50 text-xs"
              :class="weekendLeaveDeduction === option.value ? 'bg-blue-50 border-blue-500' : ''">
              <input type="radio" v-model="weekendLeaveDeduction" :value="option.value" class="mr-2" @change="saveWeekendSettings" />
              <div>
      ...
      <div v-if="weekendTab === 'department'" class="space-y-4">
        <select v-model="selectedDepartment" @change="loadDepartmentWeekend" class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded">
        ...
        <div v-if="selectedDepartment">
          ...
          <div class="flex flex-wrap gap-2">
            <label v-for="day in weekDays" :key="day.value" class="flex items-center px-3 py-1.5 border rounded cursor-pointer text-xs"
              :class="departmentWeekendDays.includes(day.value) ? 'bg-blue-100 border-blue-500' : 'bg-white border-gray-300'">
              <input type="checkbox" :value="day.value" v-model="departmentWeekendDays" class="mr-2 w-3 h-3" @change="saveDepartmentWeekend" />
              {{ day.label }}
            </label>
          </div>
        </div>
      ...
      <div v-if="weekendTab === 'employee'" class="space-y-4">
        <select v-model="selectedEmployee" @change="loadEmployeeWeekend" class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded">
          <option value="">Çalışan Seçiniz</option>
          <option v-for="emp in employees" :key="emp._id" :value="emp._id">{{ emp.firstName }} {{ emp.lastName }}</option>
        </select>
        <div v-if="selectedEmployee">
          <p class="text-[10px] text-gray-500 mb-2">Boş bırakılırsa departman/şirket ayarları kullanılır</p>
          <div class="flex flex-wrap gap-2">
            <label v-for="day in weekDays" :key="day.value" class="flex items-center px-3 py-1.5 border rounded cursor-pointer text-xs"
              :class="employeeWeekendDays.includes(day.value) ? 'bg-blue-100 border-blue-500' : 'bg-white border-gray-300'">
              <input type="checkbox" :value="day.value" v-model="employeeWeekendDays" class="mr-2 w-3 h-3" @change="saveEmployeeWeekend" />
              {{ day.label }}
            </label>
          </div>
        </div>
    ...
    <div v-show="activeTab === 'holidays'" class="bg-white rounded-lg shadow p-4">
      ...
      <div v-else class="space-y-3">
        <div class="flex gap-2">
          <select v-model="holidayYear" @change="loadHolidays" class="px-2 py-1.5 text-xs border border-gray-300 rounded">
          ...
          <table class="min-w-full text-xs">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="holiday in holidays" :key="holiday">
                ...
                <td class="px-3 py-2">
                  <button @click="deleteHoliday(holiday)" class="text-red-600 hover:text-red-800">Sil</button>
                </td>
              </tr>
        ...
        <table class="min-w-full text-xs">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="wh in workingHoursList" :key="wh._id">
              ...
              <td class="px-3 py-2">
                <button @click="deleteWorkingHours(wh._id)" class="text-red-600 hover:text-red-800">Sil</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-show="activeTab === 'puantaj'" class="bg-white rounded-lg shadow p-4">
      <div class="flex justify-between items-center mb-4">
        ...
        <router-link to="/puantaj-templates" class="text-xs text-blue-600 hover:text-blue-800">Tümünü Yönet →</router-link>
      </div>
      <div class="overflow-x-auto">
        <table class="min-w-full text-xs">
          <thead class="bg-gray-50">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="pt in puantajTemplates" :key="pt._id">
              ...
              <td class="px-3 py-2">
                <span :class="pt.isDefault ? 'text-green-600' : 'text-gray-500'">{{ pt.isDefault ? 'Varsayılan' : 'Normal' }}</span>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-show="activeTab === 'leave-approval'" class="bg-white rounded-lg shadow p-4">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
          ...
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" v-model="form.leaveApprovalSettings.enabled" @change="saveLeaveApprovalSettings" class="sr-only peer" />
            <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        </div>

        <div v-if="form.leaveApprovalSettings.enabled" class="space-y-4 pl-4 border-l-2 border-gray-200">
          <!-- Onaylayıcı Yoksa -->
          <div class="space-y-3">
            <label class="block text-xs font-medium text-gray-700">Onaylayıcı Olmadığında</label>
            <div class="space-y-2">
              <label class="flex items-center p-2 border rounded cursor-pointer hover:bg-gray-50" :class="!form.leaveApprovalSettings.autoApproveIfNoApprover ? 'bg-blue-50 border-blue-500' : ''">
                <input type="radio" v-model="form.leaveApprovalSettings.autoApproveIfNoApprover" :value="false" @change="saveLeaveApprovalSettings" class="mr-2" />
                <div>
                  <span class="text-xs font-medium">Onay Bekle</span>
                  <p class="text-[10px] text-gray-500">Talep PENDING durumunda beklesin (yönetici manuel müdahale edecek)</p>
                </div>
              </label>
              <label class="flex items-center p-2 border rounded cursor-pointer hover:bg-gray-50" :class="form.leaveApprovalSettings.autoApproveIfNoApprover ? 'bg-blue-50 border-blue-500' : ''">
                <input type="radio" v-model="form.leaveApprovalSettings.autoApproveIfNoApprover" :value="true" @change="saveLeaveApprovalSettings" class="mr-2" />
                <div>
          ...
          <div>
            ...
            <Input v-model.number="form.leaveApprovalSettings.approvalLevels" type="number" :min="0" @blur="saveLeaveApprovalSettings" />
            <p class="text-[10px] text-gray-500 mt-1">0 = Tüm onay zinciri, 1+ = Belirli sayıda onay yeterli</p>
          </div>
          ...
          <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="form.leaveApprovalSettings.allowSelfApproval" @change="saveLeaveApprovalSettings" class="sr-only peer" />
              <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>
            ...
            <div class="flex">
              ...
              <div>
                ...
                <p class="text-[10px] text-blue-600 mt-1">
                  Onay zinciri çalışanın departman yöneticisi, üst departman yöneticileri ve direkt yöneticisinden oluşur.
                </p>
              </div>
            </div>
          </div>
    ...
    <div v-show="activeTab === 'advance'" class="bg-white rounded-lg shadow p-4">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
          <div>
            <label class="block text-xs font-medium text-gray-700">Avans Talebi Sistemini Aktifleştir</label>
            <p class="text-[10px] text-gray-500">Çalışanların avans talep edebilmesini sağlar</p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" v-model="form.advanceSettings.enabled" @change="saveAdvanceSettings" class="sr-only peer" />
            <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        </div>
        ...
        <div v-if="form.advanceSettings.enabled" class="space-y-4 pl-4 border-l-2 border-gray-200">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Maksimum Tutar Tipi</label>
              <select v-model="form.advanceSettings.maxAmountType" @change="saveAdvanceSettings" class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div v-if="form.advanceSettings.maxAmountType !== 'UNLIMITED'">
              ...
              <Input v-model.number="form.advanceSettings.maxAmountValue" type="number" :min="1" @blur="saveAdvanceSettings" />
            </div>
          </div>
          ...
          <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
            <div>
              <label class="block text-xs font-medium text-gray-700">Taksitli Avansa İzin Ver</label>
              <p class="text-[10px] text-gray-500">Çalışanlar avansı taksitle geri ödeyebilsin</p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="form.advanceSettings.allowInstallments" @change="saveAdvanceSettings" class="sr-only peer" />
              <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>

          <div v-if="form.advanceSettings.allowInstallments">
            <label class="block text-xs font-medium text-gray-700 mb-1">Maksimum Taksit Sayısı</label>
            <Input v-model.number="form.advanceSettings.maxInstallments" type="number" :min="1" :max="12" @blur="saveAdvanceSettings" />
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Minimum Çalışma Süresi (Ay)</label>
              <Input v-model.number="form.advanceSettings.minWorkMonths" type="number" :min="0" @blur="saveAdvanceSettings" />
            ...
            <div>
              ...
              <Input v-model.number="form.advanceSettings.monthlyRequestLimit" type="number" :min="0" @blur="saveAdvanceSettings" />
          ...
          <div>
            ...
            <Input v-model.number="form.advanceSettings.requestStartDay" type="number" :min="0" :max="31" @blur="saveAdvanceSettings" />
            <p class="text-[10px] text-gray-500 mt-1">Ayın kaçıncı gününden itibaren talep alınsın (0 = Her zaman)</p>
          </div>
          ...
          <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="form.advanceSettings.approvalRequired" @change="saveAdvanceSettings" class="sr-only peer" />
              <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>
          ...
          <div class="bg-blue-50 border-l-4 border-blue-500 p-3 mt-4">
            <div class="flex">
              ...
              <div>
                ...
                <p class="text-[10px] text-blue-600 mt-1">
                  Avans sistemi çalışanlarınızın maaş avansı talep edebilmesini sağlar. Talepler onay zincirine gider ve ödeme planı oluşturulur.
                </p>
              </div>
            </div>
          </div>
    ...
    <div v-show="activeTab === 'advance-approval'" class="bg-white rounded-lg shadow p-4">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
          ...
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" v-model="form.advanceApprovalSettings.enabled" @change="saveAdvanceApprovalSettings" class="sr-only peer" />
            <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
        </div>

        <div v-if="form.advanceApprovalSettings.enabled" class="space-y-4 pl-4 border-l-2 border-gray-200">
          ...
          <div class="flex items-center justify-between p-3 bg-yellow-50 rounded">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="form.advanceApprovalSettings.useLeaveApprovalChain" @change="saveAdvanceApprovalSettings" class="sr-only peer" />
          ...
          <div class="space-y-3">
            <label class="block text-xs font-medium text-gray-700">Onaylayıcı Olmadığında</label>
            <div class="space-y-2">
              <label class="flex items-center p-2 border rounded cursor-pointer hover:bg-gray-50" :class="!form.advanceApprovalSettings.autoApproveIfNoApprover ? 'bg-blue-50 border-blue-500' : ''">
                <input type="radio" v-model="form.advanceApprovalSettings.autoApproveIfNoApprover" :value="false" @change="saveAdvanceApprovalSettings" class="mr-2" />
                <div>
                  <span class="text-xs font-medium">Onay Bekle</span>
              ...
              <label class="flex items-center p-2 border rounded cursor-pointer hover:bg-gray-50" :class="form.advanceApprovalSettings.autoApproveIfNoApprover ? 'bg-blue-50 border-blue-500' : ''">
                <input type="radio" v-model="form.advanceApprovalSettings.autoApproveIfNoApprover" :value="true" @change="saveAdvanceApprovalSettings" class="mr-2" />
                <div>
                  <span class="text-xs font-medium">Otomatik Onayla</span>
                  <p class="text-[10px] text-gray-500">Onaylayıcı yoksa talep direkt onaylansın</p>
                </div>
          ...
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">Kaç Seviye Onay Gerekli</label>
            <Input v-model.number="form.advanceApprovalSettings.approvalLevels" type="number" :min="0" @blur="saveAdvanceApprovalSettings" />
            <p class="text-[10px] text-gray-500 mt-1">0 = Tüm onay zinciri, 1+ = Belirli sayıda onay yeterli</p>
          </div>
          ...
          <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" v-model="form.advanceApprovalSettings.allowSelfApproval" @change="saveAdvanceApprovalSettings" class="sr-only peer" />
              <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          </div>
          ...
          <div class="bg-blue-50 border-l-4 border-blue-500 p-3">
            <div class="flex">
              ...
              <div>
                ...
                <p class="text-[10px] text-blue-600 mt-1">
                  İzin onay akışını kullanırsan, izin talepleriyle aynı yöneticiler avans taleplerini de onaylar. Kapatırsan özel avans onay zinciri kullanılır (departman yöneticisi → İK yöneticisi → şirket admini).
                </p>
              </div>
            </div>
          </div>
    ...
    <div v-show="activeTab === 'additional-payments'" class="bg-white rounded-lg shadow">
      ...
      <div class="border-b border-gray-200 px-4 pt-3">
        <nav class="flex gap-1">
          <button
            ...
            :class="paymentSubTab === 'company-payments'
          ...
          <button
            ...
            class="px-4 py-2 text-xs font-medium rounded-t-lg transition-colors border-b-2"
            :class="paymentSubTab === 'payment-types'
          ...
          <button
            ...
            class="px-4 py-2 text-xs font-medium rounded-t-lg transition-colors border-b-2"
            :class="paymentSubTab === 'deduction-types'
      ...
      <div class="p-4">
        ...
        <div v-show="paymentSubTab === 'company-payments'">
          ...
          <p class="text-xs text-gray-500 mb-4">
            Şirketinizde kullanılacak ek ödeme türlerini aktifleştirin ve varsayılan tutarlarını belirleyin.
            Çalışanlara ödeme atarken bu varsayılan değerler kullanılır veya özelleştirilebilir.
          </p>
      ...
      <div class="space-y-2">
        <div
          ...
          :class="isPaymentTypeEnabled(pt._id) ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'"
        >
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-200 text-gray-600">{{ pt.code }}</span>
                  ...
                  <span v-if="pt.isTaxExempt" class="text-[10px] px-1.5 py-0.5 rounded bg-green-100 text-green-700">
                    Vergi Muaf
                  </span>
                </div>
                <p v-if="pt.description" class="text-xs text-gray-500 mt-0.5">{{ pt.description }}</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div v-if="isPaymentTypeEnabled(pt._id)" class="text-right">
                <div class="text-sm font-semibold text-green-700">
                  {{ formatCurrency(getCompanyPaymentType(pt._id)?.companySettings?.defaultAmount || getCompanyPaymentType(pt._id)?.defaultAmount) }}
                </div>
                <div v-if="getCompanyPaymentType(pt._id)?.companySettings?.customName || getCompanyPaymentType(pt._id)?.customName" class="text-[10px] text-gray-500">
                  {{ getCompanyPaymentType(pt._id)?.companySettings?.customName || getCompanyPaymentType(pt._id)?.customName }}
                </div>
              </div>
              ...
              <button
                ...
                @click="openPaymentTypeModal(pt, getCompanyPaymentType(pt._id)?.companySettings || getCompanyPaymentType(pt._id))"
          ...
          <!-- Ek Bilgiler -->
          <div v-if="pt.isTaxExempt && pt.taxExemptLimit > 0" class="mt-2 text-[10px] text-gray-500 pl-12">
          ...
          <!-- Boş Durum -->
          <div v-if="!loadingPaymentTypes && additionalPaymentTypes.length === 0" class="text-center py-8 text-gray-500">
            <svg class="w-12 h-12 mx-auto mb-3 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p class="text-sm">Henüz ödeme türü tanımlanmamış</p>
            <p class="text-xs mt-1">Sistem yöneticisi tarafından ödeme türleri eklendikten sonra burada görünecekler</p>
          </div>
        </div>
        ...
        <div v-show="paymentSubTab === 'payment-types'" v-if="isSuperAdmin">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">Sistemdeki tüm ödeme türlerini yönetin. Bu türler tüm şirketler tarafından kullanılabilir.</p>
            </div>
            <button
          ...
          <div v-else class="space-y-2">
            <div
              ...
              :key="pt._id"
              class="border rounded-lg p-3 transition-colors"
              :class="pt.isActive ? 'bg-white border-gray-200' : 'bg-gray-100 border-gray-300 opacity-60'"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      <span v-if="!pt.isActive" class="text-[10px] px-1.5 py-0.5 rounded bg-red-100 text-red-700">
                        Pasif
                      </span>
                    </div>
                    <p v-if="pt.description" class="text-xs text-gray-500 mt-0.5">{{ pt.description }}</p>
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button
                    ...
                    :class="pt.isActive ? 'text-gray-400 hover:text-red-600 hover:bg-red-50' : 'text-gray-400 hover:text-green-600 hover:bg-green-50'"
                    :title="pt.isActive ? 'Pasif Yap' : 'Aktif Yap'"
                  >
                    <svg v-if="pt.isActive" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-if="!loadingPaymentTypes && additionalPaymentTypes.length === 0" class="text-center py-8 text-gray-500">
            ...
            <p class="text-sm">Henüz ödeme türü tanımlanmamış</p>
            <p class="text-xs mt-1">"Yeni Tür Ekle" butonuna tıklayarak ödeme türü ekleyebilirsiniz</p>
          </div>
        </div>
        ...
        <div v-show="paymentSubTab === 'deduction-types'">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">Çalışanlardan yapılacak kesintilerin türlerini yönetin.</p>
            </div>
            <button
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-200 text-gray-600">{{ dt.code }}</span>
                      <span class="text-[10px] px-1.5 py-0.5 rounded"
                            :class="{
                    ...
                    <p v-if="dt.description" class="text-xs text-gray-500 mt-0.5">{{ dt.description }}</p>
                  </div>
                </div>

                <div class="flex items-center gap-2">
                  <div v-if="dt.defaultAmount" class="text-right mr-2">
                    <span class="text-sm font-semibold text-red-600">{{ formatCurrency(dt.defaultAmount) }}</span>
                  </div>
                  <button
          ...
          <!-- Boş Durum -->
          <div v-if="!loadingDeductionTypes && deductionTypes.length === 0" class="text-center py-8 text-gray-500">
            <svg class="w-12 h-12 mx-auto mb-3 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
            </svg>
            <p class="text-sm">Henüz kesinti türü tanımlanmamış</p>
            <p class="text-xs mt-1">"Yeni Kesinti Türü" butonuna tıklayarak kesinti türü ekleyebilirsiniz</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showPaymentTypeModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto py-4">
      <div class="bg-white rounded-lg p-5 w-full max-w-2xl mx-4">
        <h3 class="text-sm font-bold mb-4">
          {{ editingPaymentType?.name }} - Ayarlar
        </h3>

        <form @submit.prevent="savePaymentType" class="space-y-4">
          <!-- Ödeme Yöntemi Seçenekleri (Yemek Kazancı için) -->
          <div v-if="editingPaymentType?.hasPaymentMethodOptions && editingPaymentType?.paymentMethodOptions?.length > 0" class="border rounded-lg p-4 bg-gray-50">
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <label :for="'method_' + method.code" class="flex-1 cursor-pointer">
                  <span class="text-sm font-medium text-gray-800">{{ editingPaymentType.name }} {{ method.name.toLowerCase() }}</span>
            ...
            <div v-if="paymentTypeForm.selectedPaymentMethod === 'SUBSCRIPTION'" class="mt-3 flex items-center gap-2 p-2 bg-orange-50 rounded-lg border border-orange-200">
              <input
            ...
            <div v-if="activeYearlyTaxLimit" class="mt-4 p-3 bg-blue-50 rounded-lg">
              <p class="text-xs font-medium text-blue-800 mb-1">{{ activeYearlyTaxLimit.year }} Yılı Limitleri:</p>
              <!-- Yemek için limitler -->
              <div v-if="editingPaymentType?.code === 'YEMEK'" class="flex gap-4 text-[11px] text-blue-700">
                <span>SGK Günlük Limit: {{ formatCurrency(activeYearlyTaxLimit.yemekLimits?.sgkDailyLimit) }}</span>
                <span>Vergi Günlük Limit: {{ formatCurrency(activeYearlyTaxLimit.yemekLimits?.vatDailyLimit) }}</span>
              </div>
              <!-- Yol için limitler -->
              <div v-else-if="editingPaymentType?.code === 'YOL'" class="flex flex-col gap-1 text-[11px] text-blue-700">
                <div class="flex gap-4">
                  <span>Günlük Limit (Abonman): {{ formatCurrency(activeYearlyTaxLimit.yolLimits?.sgkDailyLimit) }}</span>
                  <span v-if="paymentTypeForm.includesVat">KDV Dahil: {{ formatCurrency((activeYearlyTaxLimit.yolLimits?.sgkDailyLimit || 0) * 1.10) }}</span>
                </div>
              </div>
              <p class="text-[10px] text-blue-600 mt-1 italic">
                Not: Ek Kazanç ve Kesinti tanımları dönemden bağımsız yapılır. Bu nedenle önceki yıl bordrolarını hesapladıktan ve kesinleştirdikten sonra bu ekran ile Ek Kazanç ve Kesintileri düzenleyebilirsiniz.
              </p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <p class="text-[10px] text-gray-500 mt-1">Çalışanlara atanırken kullanılacak varsayılan tutar</p>
            </div>

            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Özel İsim (İsteğe Bağlı)</label>
              <input
              ...
              />
              <p class="text-[10px] text-gray-500 mt-1">Şirketinize özel isim kullanmak isterseniz</p>
            </div>
          </div>

          <div v-if="editingPaymentType?.paymentFrequency === 'ONE_TIME'" class="flex items-center gap-2">
            <input
          ...
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">Maksimum Tutar (İsteğe Bağlı)</label>
            <div class="relative">
              <input
          ...
          <div class="text-[10px] text-gray-500 border-t pt-3">
            Diğer özel durumlar için lütfen ek kazanç ve kesinti ekranlarında düzenleme yapınız. Seçili bölümler için kazanç kesinti bilgileri kaydetme işlemi ile otomatik ayarlanacaktır.
          </div>

          <div class="flex gap-2 justify-end pt-2">
            <Button variant="secondary" size="sm" type="button" @click="closePaymentTypeModal">İptal</Button>
    ...
    <div v-show="activeTab === 'yearly-tax-limits'" class="bg-white rounded-lg shadow p-4">
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">Yemek ve Yol kazançları için yıllık SGK ve Vergi muafiyet limitlerini düzenleyin.</p>
        </div>
        <button
      ...
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              <span class="text-lg font-bold" :class="limit.isActive ? 'text-green-700' : 'text-gray-700'">
                {{ limit.year }}
              </span>
              <span
                v-if="limit.isActive"
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-lg p-3 border border-orange-100">
              ...
              <div class="space-y-1 text-xs">
                <div class="flex justify-between">
                  ...
                  <span class="font-medium text-gray-700">{{ formatCurrency(limit.yemekLimits?.sgkDailyLimit) }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-500">Vergi Günlük:</span>
                  <span class="font-medium text-gray-700">{{ formatCurrency(limit.yemekLimits?.vatDailyLimit) }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-lg p-3 border border-blue-100">
              ...
              <div class="space-y-1 text-xs">
                <div class="flex justify-between">
                  <span class="text-gray-500">SGK Günlük:</span>
                  <span class="font-medium text-gray-700">{{ formatCurrency(limit.yolLimits?.sgkDailyLimit) }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-500">Vergi Günlük:</span>
                  <span class="font-medium text-gray-700">{{ formatCurrency(limit.yolLimits?.vatDailyLimit) }}</span>
                </div>
                <div class="flex justify-between border-t pt-1 mt-1">
                  <span class="text-gray-500">SGK Aylık:</span>
                  <span class="font-medium text-gray-700">{{ formatCurrency(limit.yolLimits?.sgkMonthlyLimit) }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-500">Vergi Aylık:</span>
                  <span class="font-medium text-gray-700">{{ formatCurrency(limit.yolLimits?.vatMonthlyLimit) }}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div v-if="yearlyTaxLimits.length === 0" class="text-center py-8 text-gray-500">
          ...
          <p class="text-xs mt-1">Yeni yıl ekleyerek SGK ve Vergi limitlerini tanımlayabilirsiniz</p>
        </div>
      </div>
    </div>

    <!-- Yıllık Limit Düzenleme Modal -->
    <div v-if="showYearlyLimitModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
    ...
    <div v-show="activeTab === 'password'" class="bg-white rounded-lg shadow p-4 max-w-md">
      ...
      <form @submit.prevent="changePassword" class="space-y-3">
        <div v-if="authStore.user?.mustChangePassword" class="bg-blue-50 border border-blue-200 text-blue-700 px-3 py-2 rounded text-xs">
          İlk giriş yapıyorsunuz. Lütfen şifrenizi belirleyin.
        </div>
        <div v-if="!authStore.user?.mustChangePassword">
          <Input v-model="passwordForm.currentPassword" type="password" label="Mevcut Şifre" required />
        </div>
        <Input v-model="passwordForm.newPassword" type="password" label="Yeni Şifre" required minlength="6" />
        <Input v-model="passwordForm.confirmPassword" type="password" label="Yeni Şifre Tekrar" required />
        <div v-if="passwordForm.newPassword && passwordForm.confirmPassword && passwordForm.newPassword !== passwordForm.confirmPassword" class="text-xs text-red-600">
        ...
        <div class="flex justify-end">
          <Button type="submit" size="sm" :disabled="passwordForm.newPassword !== passwordForm.confirmPassword || passwordForm.newPassword.length < 6 || changingPassword">
    ...
    <!-- Tatil Ekleme Modal -->
    <div v-if="showAddHolidayModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-4 w-full max-w-sm">
        ...
        <form @submit.prevent="addHoliday" class="space-y-3">
          ...
          <div class="flex gap-2 justify-end">
            <Button variant="secondary" size="sm" @click="showAddHolidayModal = false">İptal</Button>
    ...
    <div v-if="showAddWorkingHoursModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-4 w-full max-w-sm">
        ...
        <form @submit.prevent="addWorkingHours" class="space-y-3">
          ...
          <Input v-model.number="newWorkingHours.breakDuration" type="number" label="Mola Süresi (dk)" />
          <div class="flex gap-2 justify-end">
            <Button variant="secondary" size="sm" @click="showAddWorkingHoursModal = false">İptal</Button>
            <Button type="submit" size="sm">Ekle</Button>
          </div>
        </form>
      </div>
...
const tabIcons = {
  theme: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" /></svg>',
  company: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>',
  ...
  departments: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" /><rect x="3" y="14" width="7" height="7" rx="1" /><rect x="14" y="14" width="7" height="7" rx="1" /></svg>',
  users: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>',
  checkin: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4" /><polyline points="10 17 15 12 10 7" /><line x1="15" y1="12" x2="3" y2="12" /></svg>',
  weekend: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /><path d="M8 14h.01M12 14h.01M16 14h.01M8 18h.01M12 18h.01" /></svg>',
  holidays: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /><path d="M12 14l1.5 3 3.5.5-2.5 2.5.5 3.5-3-1.5-3 1.5.5-3.5-2.5-2.5 3.5-.5z" /></svg>',
  workinghours: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>',
  puantaj: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><line x1="3" y1="9" x2="21" y2="9" /><line x1="3" y1="15" x2="21" y2="15" /><line x1="9" y1="3" x2="9" y2="21" /><line x1="15" y1="3" x2="15" y2="21" /></svg>',
  'leave-approval': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M9 11l3 3L22 4" /><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" /></svg>',
  advance: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M20 12V8H6a2 2 0 01-2-2c0-1.1.9-2 2-2h12v4" /><path d="M4 6v12a2 2 0 002 2h14v-4" /><path d="M18 12a2 2 0 00-2 2v4h4v-4a2 2 0 00-2-2z" /></svg>',
  'advance-approval': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M12 2v4m0 12v4M4.93 4.93l2.83 2.83m8.48 8.48l2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48l2.83-2.83" /><circle cx="12" cy="12" r="4" /></svg>',
  password: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg>',
  dealer: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>',
  'additional-payments': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M12 2v2m0 16v2M4.93 4.93l1.41 1.41m11.32 11.32l1.41 1.41M2 12h2m16 0h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41M12 8a4 4 0 100 8 4 4 0 000-8z" /><path d="M12 10v4m-2-2h4" /></svg>',
  'yearly-tax-limits': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>'
}
...
const tabs = computed(() => {
  ...
  if (authStore.hasAnyRole('super_admin', 'bayi_admin')) {
    baseTabs.push({ id: 'yearly-tax-limits', name: 'Yıllık Limitler', icon: tabIcons['yearly-tax-limits'] })
  }
...
const leaveDeductionOptions = [
  { value: 'none', label: 'Hiçbiri düşülmesin', desc: 'Hafta tatili günleri yıllık izinden düşülmez' },
  { value: 'all', label: 'Tümü düşülsün', desc: 'Tüm hafta tatili günleri yıllık izinden düşülür' },
  { value: 'first_only', label: 'Sadece birinci gün', desc: 'Sadece birinci hafta tatili günü düşülür' },
  { value: 'second_only', label: 'Sadece ikinci gün', desc: 'Sadece ikinci hafta tatili günü düşülür' }
]
  ...
  try {
    ...
    if (response.data.checkInSettings) {
      form.value.checkInSettings = {
        ...
        allowedLocation: response.data.checkInSettings.allowedLocation || { latitude: null, longitude: null, radius: 100 },
    ...
    if (companyId) {
      ...
      if (companyResponse.data.leaveApprovalSettings) {
        form.value.leaveApprovalSettings = {
          ...
          autoApproveIfNoApprover: companyResponse.data.leaveApprovalSettings.autoApproveIfNoApprover || false,
      ...
      if (companyResponse.data.advanceApprovalSettings) {
        form.value.advanceApprovalSettings = {
          ...
          useLeaveApprovalChain: companyResponse.data.advanceApprovalSettings.useLeaveApprovalChain !== false,
          requireApproval: companyResponse.data.advanceApprovalSettings.requireApproval !== false,
          autoApproveIfNoApprover: companyResponse.data.advanceApprovalSettings.autoApproveIfNoApprover || false,
          approvalLevels: companyResponse.data.advanceApprovalSettings.approvalLevels || 0,
          allowSelfApproval: companyResponse.data.advanceApprovalSettings.allowSelfApproval || false
        }
      }
    }
...
const getCompanyPaymentType = (paymentTypeId) => {
  return companyPaymentTypes.value.find(cpt =>
...
const openPaymentTypeModal = (paymentType, companySettings = null) => {
  ...
  if (companySettings) {
    ...
    const defaultMethod = paymentType.hasPaymentMethodOptions && paymentType.paymentMethodOptions?.length > 0
    ...
    paymentTypeForm.value = {
      ...
      createAutoDeduction: defaultMethod === 'MEAL_CARD_ONLY' || defaultMethod === 'NON_MEAL_CARD' || defaultMethod === 'SUBSCRIPTION',
      includesVat: false
    }
  }
...
const onPaymentMethodChange = (methodCode) => {
  ...
  if (method) {
    ...
    paymentTypeForm.value.createAutoDeduction = methodCode === 'MEAL_CARD_ONLY' || methodCode === 'NON_MEAL_CARD'
  }
}
...
const saveTemplateSetting = async () => {
  try {
    ...
    await api.put(`/companies/${companyId}/attendance-template`, { templateId: form.value.activeAttendanceTemplate || null })
...
const savePayrollType = async () => {
  try {
    ...
    await api.put(`/companies/${companyId}/payroll-calculation-type`, { payrollCalculationType: form.value.payrollCalculationType })
...
const saveAutoAddSetting = async () => {
  try {
    ...
    await api.put(`/companies/${companyId}`, { autoAddApprovedEmployees: form.value.autoAddApprovedEmployees })
...
const handleFileChange = (event) => {
  ...
  if (selectedFile.size > MAX_LOGO_SIZE) {
    toast.error(`Logo dosyası çok büyük. Maksimum ${MAX_LOGO_SIZE / 1024}KB olmalıdır. Seçilen dosya: ${Math.round(selectedFile.size / 1024)}KB`)
