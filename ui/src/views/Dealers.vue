    ...
    <div class="bg-white rounded-lg shadow p-4 mb-4">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <select v-model="filters.timeStatus" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="">Tümü</option>
        ...
        <table class="min-w-full divide-y divide-gray-200 table-fixed">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="w-[15%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bayi Adı</th>
              <th class="w-[12%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
              <th class="w-[10%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Telefon</th>
              ...
              <th class="w-[12%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kalan Süre</th>
              <th class="w-[8%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Durum</th>
              <th class="w-[8%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Maks. Şirket</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
                ...
                <div class="text-sm truncate" :title="formatRemainingTime(dealer.endDate)">
                  <span
                    :class="{
                      ...
                      'text-yellow-600 font-semibold': getRemainingDays(dealer.endDate) > 30 && getRemainingDays(dealer.endDate) <= 90,
              ...
              <td class="px-2 py-3">
                <span
                  :class="dealer.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'"
              ...
              <td class="px-2 py-3 text-sm text-gray-600 text-center">
                {{ dealer.maxCompanies !== null && dealer.maxCompanies !== undefined ? dealer.maxCompanies : '∞' }}
              </td>
              <td class="px-2 py-3">
                <div class="flex gap-1">
                  <button @click="viewCompanies(dealer._id)" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Şirketler">
                  ...
                  <button
                    ...
                    :class="dealer.isActive ? 'text-orange-600 hover:bg-orange-50' : 'text-green-600 hover:bg-green-50'"
                    class="p-1.5 rounded"
                    :title="dealer.isActive ? 'Pasif Yap' : 'Aktif Yap'"
                  >
                    <svg v-if="dealer.isActive" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button @click="editDealer(dealer)" class="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded" title="Düzenle">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                  </button>
                  <button @click="deleteDealer(dealer._id)" class="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Sil">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="p-4 bg-gray-50 border-t">
        <div class="flex justify-between items-center">
          <p class="text-sm text-gray-600">
            Toplam {{ filteredDealers.length }} bayi gösteriliyor
          </p>
          <div class="flex gap-4 text-sm text-gray-600">
            ...
            <span>Süresi Yakın: <strong class="text-yellow-600">{{ expiringCount }}</strong></span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <form @submit.prevent="saveDealer">
          <div class="space-y-4">
            ...
            <Input v-model="form.contactPerson" label="Yetkili Kişi" placeholder="Ad Soyad" />
            <Input v-model="form.taxNumber" label="Vergi No / TCKN" placeholder="Ödeme için gerekli" />
            <Textarea v-model="form.address" label="Adres" />
            <div class="grid grid-cols-2 gap-4">
              <Input v-model="form.city" label="Şehir" placeholder="Istanbul" />
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1">
                Varsayılan: Başlangıç tarihinden 1 yıl sonra
              </p>
            </div>
...
const filteredDealers = computed(() => {
  ...
  if (filters.value.timeStatus === 'expired') {
    ...
    result = result.filter(d => getRemainingDays(d.endDate) >= 0 && getRemainingDays(d.endDate) <= 30)
...
const expiringCount = computed(() => dealers.value.filter(d => {
