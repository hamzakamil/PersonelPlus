  ...
  <div class="p-4">
    ...
    <div class="flex justify-between items-center mb-4">
      <div class="flex items-center gap-3">
        <select v-model="filters.year" class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          <option value="">Tüm Yıllar</option>
          <option v-for="y in availableYears" :key="y" :value="y">{{ y }}</option>
        </select>
        <select v-model="filters.month" :disabled="!filters.year" class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed">
    ...
    <div class="bg-white rounded-lg shadow p-3 mb-3">
      <div class="flex flex-wrap items-end gap-3">
        ...
        <div v-if="isBayiAdmin" class="min-w-[160px]">
          ...
          <select v-model="filters.company" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
        ...
        <div class="min-w-[140px]">
          ...
          <select v-model="filters.department" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
            <option value="">Tümü</option>
            <option v-for="dept in allDepartments" :key="dept._id" :value="dept._id">{{ dept.name }}</option>
          </select>
        </div>
        <div class="min-w-[100px]">
          <label class="block text-xs font-medium text-gray-600 mb-1">Durum</label>
          <select v-model="filters.status" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
            <option value="">Tümü</option>
            <option value="active">Aktif</option>
            <option value="separated">Ayrılmış</option>
          </select>
        </div>
        <button @click="resetFilters" class="px-3 py-1.5 text-xs text-gray-500 hover:text-gray-700 border border-gray-300 rounded hover:bg-gray-50">
          Temizle
        </button>
      </div>
    </div>
    ...
    <div v-if="selectedEmployees.length > 0" class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="w-10 px-1 py-2 text-left text-[10px] font-medium text-gray-500 uppercase">#</th>
              <th @click="toggleSort('name')" class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                <div class="flex items-center gap-1">
                  Ad Soyad
                  <svg class="w-3 h-3" :class="sortField === 'name' ? 'text-blue-600' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                    <path v-if="sortField === 'name' && sortDirection === 'asc'" d="M5.293 7.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L10 4.414l-3.293 3.293a1 1 0 01-1.414 0z" />
                    <path v-else-if="sortField === 'name' && sortDirection === 'desc'" d="M14.707 12.293a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L10 15.586l3.293-3.293a1 1 0 011.414 0z" />
                    <path v-else d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" />
                  </svg>
                </div>
              </th>
              <th class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase">TCKN</th>
              <th @click="toggleSort('position')" class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                <div class="flex items-center gap-1">
                  Görev
                  <svg class="w-3 h-3" :class="sortField === 'position' ? 'text-blue-600' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                    <path v-if="sortField === 'position' && sortDirection === 'asc'" d="M5.293 7.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L10 4.414l-3.293 3.293a1 1 0 01-1.414 0z" />
                    <path v-else-if="sortField === 'position' && sortDirection === 'desc'" d="M14.707 12.293a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L10 15.586l3.293-3.293a1 1 0 011.414 0z" />
                    <path v-else d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" />
                  </svg>
                </div>
              </th>
              <th @click="toggleSort('department')" class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                <div class="flex items-center gap-1">
                  Departman
                  <svg class="w-3 h-3" :class="sortField === 'department' ? 'text-blue-600' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                    <path v-if="sortField === 'department' && sortDirection === 'asc'" d="M5.293 7.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L10 4.414l-3.293 3.293a1 1 0 01-1.414 0z" />
                    <path v-else-if="sortField === 'department' && sortDirection === 'desc'" d="M14.707 12.293a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L10 15.586l3.293-3.293a1 1 0 011.414 0z" />
                    <path v-else d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" />
                  </svg>
                </div>
              </th>
              <th class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase">Telefon</th>
              <th class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase">Email</th>
              <th @click="toggleSort('hireDate')" class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                <div class="flex items-center gap-1">
                  İşe Giriş
                  <svg class="w-3 h-3" :class="sortField === 'hireDate' ? 'text-blue-600' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                    <path v-if="sortField === 'hireDate' && sortDirection === 'asc'" d="M5.293 7.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L10 4.414l-3.293 3.293a1 1 0 01-1.414 0z" />
                    <path v-else-if="sortField === 'hireDate' && sortDirection === 'desc'" d="M14.707 12.293a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L10 15.586l3.293-3.293a1 1 0 011.414 0z" />
                    <path v-else d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z" />
                  </svg>
                </div>
              </th>
              <th class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase">Emeklilik</th>
              <th class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase">Durum</th>
              <th class="px-2 py-2 text-left text-[10px] font-medium text-gray-500 uppercase">İşlem</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="bg-gradient-to-r from-slate-100 to-slate-50 sticky">
                  <td :colspan="12" class="px-3 py-1.5">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-xs font-semibold text-slate-700">{{ group.companyName }}</span>
                      <span class="px-1.5 py-0.5 text-[10px] bg-slate-200 text-slate-600 rounded-full">{{ group.employees.length }} kişi</span>
                      <span v-if="group.isDealerSelfCompany" class="px-1.5 py-0.5 text-[10px] bg-purple-100 text-purple-700 rounded-full">Kendi Şirketim</span>
                    </div>
                  </td>
                </tr>
                <!-- Çalışan Satırları -->
                <tr v-for="(employee, index) in group.employees" :key="employee._id" :class="{
                  ...
                  <td class="px-2 py-1.5">
                    <div class="text-xs font-medium text-gray-900 truncate max-w-[140px]" :title="`${employee.firstName} ${employee.lastName}`">
                      {{ employee.firstName }} {{ employee.lastName }}
                    </div>
                  </td>
                  <td class="px-2 py-1.5 text-[10px] text-gray-600 font-mono">{{ employee.tcKimlik || '-' }}</td>
                  <td class="px-2 py-1.5">
                    <div class="text-[10px] text-gray-600 truncate max-w-[100px]" :title="employee.position">{{ employee.position || '-' }}</div>
                  </td>
                  <td class="px-2 py-1.5">
                    <div class="text-[10px] text-gray-600 truncate max-w-[90px]" :title="employee.department?.name">{{ employee.department?.name || '-' }}</div>
                  </td>
                  <td class="px-2 py-1.5 text-[10px] text-gray-600">{{ employee.phone || '-' }}</td>
                  <td class="px-2 py-1.5">
                    <div class="text-[10px] text-gray-600 truncate max-w-[120px] lowercase" :title="employee.email">{{ employee.email || '-' }}</div>
                  </td>
                  <td class="px-2 py-1.5 text-[10px] text-gray-600">{{ employee.hireDate ? formatDate(employee.hireDate) : '-' }}</td>
                  <td class="px-2 py-1.5">
                    <span :class="employee.isRetired ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-600'" class="px-1 py-0.5 text-[9px] font-medium rounded">
                      {{ employee.isRetired ? 'Emekli' : 'Normal' }}
                    </span>
                  </td>
                  <td class="px-2 py-1.5">
                    <span v-if="employee.status === 'separated'" class="px-1 py-0.5 text-[9px] font-medium rounded bg-red-100 text-red-700">Ayrılmış</span>
                    <span v-else-if="employee.isActivated" class="px-1 py-0.5 text-[9px] font-medium rounded bg-green-100 text-green-700">Aktif</span>
                    <span v-else class="px-1 py-0.5 text-[9px] font-medium rounded bg-yellow-100 text-yellow-700">Bekliyor</span>
                  </td>
                  <td class="px-2 py-1.5">
                    <div class="flex gap-0.5">
                      <button @click="editEmployee(employee)" class="p-1 text-indigo-600 hover:bg-indigo-50 rounded" title="Düzenle">
                      ...
                      <button @click="deleteEmployee(employee._id)" class="p-1 text-red-600 hover:bg-red-50 rounded" title="Sil">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <template v-else>
              <tr v-for="(employee, index) in sortedEmployees" :key="employee._id" :class="{
                'bg-blue-50': isSelected(employee._id),
                'bg-red-50': !isSelected(employee._id) && employee.status === 'separated',
                'hover:bg-gray-50': !isSelected(employee._id) && employee.status !== 'separated'
              }">
      ...
      <div class="px-3 py-2 bg-gray-50 border-t">
        <p class="text-xs text-gray-600">
          Toplam {{ isBayiAdmin ? groupedEmployees.reduce((acc, g) => acc + g.employees.length, 0) : sortedEmployees.length }} çalışan
        </p>
      </div>
    </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-semibold text-white">{{ editingEmployee ? 'Çalışan Düzenle' : 'Yeni Çalışan Ekle' }}</h2>
        ...
        <form @submit.prevent="saveEmployee" class="flex-1 flex flex-col overflow-hidden">
          <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div v-if="isSuperAdmin || isBayiAdmin" class="bg-purple-50 border border-purple-200 rounded-xl p-4">
              <div class="flex items-center gap-3 mb-3">
                ...
                <div>
                  <label class="text-sm font-semibold text-gray-900">Şirket Seçimi <span class="text-red-500">*</span></label>
              ...
              <div class="p-4 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 uppercase tracking-wide">Ad <span class="text-red-500">*</span></label>
                    <div class="relative">
                  ...
                  <!-- Soyad -->
                  <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 uppercase tracking-wide">Soyad <span class="text-red-500">*</span></label>
                    <div class="relative">
                      <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  ...
                  <!-- TC Kimlik No -->
                  <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 uppercase tracking-wide">TC Kimlik No <span class="text-red-500">*</span></label>
                    <div class="relative">
                      <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <p v-if="form.tcKimlik && form.tcKimlik.replace(/\D/g, '').length !== 11" class="text-[10px] text-amber-600">
                  ...
                  <!-- Email -->
                  <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 uppercase tracking-wide">E-posta</label>
                    <div class="relative">
                      <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div class="space-y-1.5">
                    <PhoneInput
                      ...
                      :error="form.phone && form.phone.length > 0 && form.phone.length !== 11 ? 'Cep numarası 11 haneli olmalıdır' : ''"
                    />
                  </div>

                  <!-- Personel Numarası -->
                  <div class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 uppercase tracking-wide">Personel No (Opsiyonel)</label>
                    <div class="relative">
                      <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="p-4 space-y-4">
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="relative" ref="jobCodeDropdownRef">
                    ...
                    >
                      ...
                      <div v-else-if="jobCodeSearch.length < 2 && !selectedJobCode" class="px-4 py-3 text-sm text-gray-500 text-center">
                      ...
                      >
                        <span class="flex-shrink-0 px-2 py-1 bg-emerald-100 text-emerald-700 text-xs font-mono rounded">
                          {{ job.kod }}
                        </span>
                  ...
                  <!-- Seçilen meslek kodu -->
                  <div v-if="selectedJobCode" class="flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <svg class="w-4 h-4 text-emerald-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span class="text-xs font-mono text-emerald-700">{{ selectedJobCode.kod }}</span>
                    <span class="text-xs text-emerald-800 flex-1 truncate">{{ selectedJobCode.ad }}</span>
                    <button type="button" @click="clearJobCode" class="text-emerald-600 hover:text-emerald-800">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    </button>
                  </div>
                  <p v-else class="text-[10px] text-gray-400">Meslek adı veya kodu yazarak arayın, listeden seçin</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div v-if="form.company && workplaces.length > 0" class="space-y-1.5">
                    ...
                    <p v-if="workplaces.length > 1 && !form.workplace" class="text-[10px] text-amber-600">
                  ...
                  <div v-if="showDepartmentField" class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 uppercase tracking-wide">Departman</label>
                    <select
                  ...
                  <div v-if="form.company" class="space-y-1.5">
                    <label class="text-xs font-semibold text-gray-700 uppercase tracking-wide">Üst Yönetici</label>
                    <select
            ...
            <div v-if="editingEmployee" class="bg-gray-50 border border-gray-200 rounded-xl p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-sm font-semibold text-gray-900 font-mono">{{ editingEmployee.employeeNumber || '-' }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 border-t px-6 py-4 flex items-center justify-end gap-3 flex-shrink-0">
            <button
              type="button"
              @click="closeModal"
...
const isIndeterminate = computed(() => {
  return selectedEmployees.value.length > 0 && selectedEmployees.value.length < filteredEmployees.value.length
})
  ...
  try {
    ...
    if (!finalWorkplace) {
      ...
      if (workplaces.value.length === 1) {
        ...
        if (isBayiAdmin.value) {
          toast.warning('Bu şirket birden fazla SGK işyerine sahiptir. Lütfen SGK işyerini seçiniz.')
        } else {
    ...
    if (!form.value.hireDate) {
      const confirmed = await confirmModal.show({
        title: 'Eksik Bilgi',
        message: 'İşe başlama tarihi girilmedi. Bu bilgi olmadan yıllık izin gün sayıları hesaplanamaz. Devam etmek istiyor musunuz?',
        type: 'warning',
    ...
    if (!form.value.birthDate) {
      const confirmed = await confirmModal.show({
        title: 'Eksik Bilgi',
        message: 'Doğum tarihi girilmedi. Bu bilgi olmadan yıllık izin gün sayıları hesaplanamaz. Devam etmek istiyor musunuz?',
        type: 'warning',
        confirmText: 'Devam Et'
      })
      if (!confirmed) return
...
const loadLeaveSubTypes = async (companyId) => {
  try {
    ...
    if (companyId) {
      ...
      params.companyId = typeof authStore.user.company === 'object' ? authStore.user.company._id : authStore.user.company
    }
...
const manualActivate = async (employeeId) => {
  const confirmed = await confirmModal.show({
    ...
    message: 'Bu çalışanı manuel olarak aktif etmek istediğinize emin misiniz?\n\nVarsayılan şifre: 123456\n(İlk girişte şifre değiştirmesi gerekecektir)',
    type: 'info',
...
const manualActivateSelected = async () => {
  ...
  try {
    const response = await api.post('/employees/bulk-manual-activate', { employeeIds: notActivatedSelected })
    const data = response.data.data
