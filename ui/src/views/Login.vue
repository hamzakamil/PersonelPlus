    ...
    <div class="max-w-md w-full bg-white rounded-lg shadow-md p-8">
      ...
      <form @submit.prevent="handleLogin">
        <div class="space-y-4">
          ...
          <!-- Fallback: Google butonu yüklenemezse -->
          <div v-if="googleError" class="text-center">
            <p class="text-red-500 text-xs mb-2">{{ googleError }}</p>
          </div>

          <div v-if="googleLoading" class="text-center">
            <p class="text-gray-500 text-sm">Google ile giriş yapılıyor...</p>
          </div>
...
const handleGoogleCredential = async (response) => {
  ...
  try {
    ...
    if (result.success) {
      router.push(result.isNewUser ? '/settings' : '/');
...
// Sayfa yüklendiğinde Google butonunu render et
const initGoogleButton = async () => {
  ...
  try {
    ...
    googleError.value = 'Google giriş servisi yüklenemedi';
  }
};
