<template>
  <div class="p-6">
        ...
        <table class="min-w-full divide-y divide-gray-200 table-fixed">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="w-[20%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
              <th class="w-[15%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rol</th>
              <th class="w-[15%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bayi</th>
              <th class="w-[15%] px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Şirket</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(user, index) in users" :key="user._id" :class="{ 'bg-gray-50': index % 2 === 1, 'bg-white': index % 2 === 0 }">
              ...
              <td class="px-2 py-3">
                <div class="text-sm text-gray-600 truncate" :title="user.role?.description || user.role?.name">
        ...
        <div class="flex justify-between items-center mb-3">
          <p class="text-sm text-gray-600">
            ...
            <span v-if="pagination.pages > 1" class="text-gray-500">
              ({{ ((pagination.page - 1) * pagination.limit) + 1 }}-{{ Math.min(pagination.page * pagination.limit, pagination.total) }} gösteriliyor)
            </span>
          </p>
          <div class="flex gap-4 text-sm text-gray-600">
            <span>Aktif: <strong class="text-green-600">{{ activeUserCount }}</strong></span>
            <span>Pasif: <strong class="text-red-600">{{ inactiveUserCount }}</strong></span>
          </div>
        </div>
    ...
    <div v-if="showRolePermissionModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <h2 class="text-xl font-bold mb-4"><span class="no-uppercase">{{ selectedUser?.email }}</span> - Rol & Yetki Yönetimi</h2>
...
const loadUsers = async () => {
  try {
    ...
    if (response.data?.success) {
      ...
      pagination.value = response.data.pagination || { page: 1, limit: 10, total: 0, totalPages: 1 }
    }
  } catch (error) {
