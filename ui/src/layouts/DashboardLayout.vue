<template>
  <div class="min-h-screen" :style="{ backgroundColor: 'var(--page-bg)' }">
    ...
    <div class="flex">
      ...
      <aside class="sidebar-fixed min-h-screen flex-shrink-0 shadow-2xl" :style="{ background: 'var(--sidebar-bg)' }">
        <!-- Logo Section -->
          ...
          <template v-for="item in menuItems" :key="item.path || item.name">
            ...
            <div v-if="item.children" class="mb-1">
              <button
                ...
                :style="{
                  color: isMenuActive(item) || isMenuExpanded(item.name) ? 'var(--sidebar-text-hover)' : 'var(--sidebar-text)',
                  backgroundColor: isMenuActive(item) || isMenuExpanded(item.name) ? 'var(--sidebar-active-bg)' : 'transparent'
                }"
                @mouseenter="$event.target.style.backgroundColor = 'var(--sidebar-active-bg)'"
                @mouseleave="$event.target.style.backgroundColor = isMenuActive(item) || isMenuExpanded(item.name) ? 'var(--sidebar-active-bg)' : 'transparent'"
              >
              ...
              <div
                ...
                class="mt-1 ml-4 space-y-1 animate-slide-up"
              >
                <router-link
                  ...
                  :style="{
                    color: $route.path === child.path ? 'var(--sidebar-text-hover)' : 'var(--sidebar-text)',
                    backgroundColor: $route.path === child.path ? 'var(--sidebar-active-bg)' : 'transparent',
                    borderLeft: $route.path === child.path ? '3px solid var(--sidebar-active)' : '3px solid transparent'
                  }"
                >
            ...
            <router-link
              ...
              :style="{
                color: $route.path === item.path ? 'var(--sidebar-text-hover)' : 'var(--sidebar-text)',
                backgroundColor: $route.path === item.path ? 'var(--sidebar-active-bg)' : 'transparent',
                borderLeft: $route.path === item.path ? '3px solid var(--sidebar-active)' : '3px solid transparent'
              }"
            >
              <span class="w-5 h-5 flex-shrink-0" v-html="item.icon"></span>
              <span class="font-medium">{{ item.name }}</span>
      ...
      <main class="flex-1 min-w-0">
        <!-- Header -->
        <header class="shadow-sm border-b sticky top-0 z-10" :style="{ backgroundColor: 'var(--header-bg)', borderColor: 'var(--header-border)' }">
          <div class="px-6 py-3 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
              <NotificationBadge />
              <MessageBadge />
              <div class="flex items-center gap-3 px-4 py-2 rounded-xl" :style="{ backgroundColor: 'var(--primary-light)' }">
                <div class="text-right">
                  <div v-if="displayRoleLabel" class="text-[10px] font-medium uppercase tracking-wide" :style="{ color: 'var(--text-muted)' }">
                    {{ displayRoleLabel }}
                  </div>
                ...
                <div class="w-9 h-9 rounded-full flex items-center justify-center text-white font-semibold text-sm" :style="{ backgroundColor: 'var(--primary)' }">
                  {{ userInitials }}
                </div>
              </div>
              <Button variant="secondary" size="sm" @click="handleLogout" class="flex items-center gap-2">
...
const displayRoleLabel = computed(() => {
  ...
  if (['company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'].includes(role)) return 'Şirket'
  return ''
})
...
const displayUserName = computed(() => {
  ...
  if (['company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'].includes(role) && user.value.company) {
...
const icons = {
  ...
  dealers: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>',
  database: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3" /><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5" /></svg>',
  ...
  globe: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z" /></svg>',
  ...
  calendar: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>',
  userCog: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><circle cx="18" cy="15" r="3" /><circle cx="9" cy="7" r="4" /><path d="M10 15H6a4 4 0 00-4 4v2" /><path d="M21.7 16.4l-.9-.3m-5.6-2.2l-.9-.3m2.3 5.1l.3-.9m2.2-5.6l.3-.9m.2 7.4l-.4-1m-2.4-5.4l-.4-1m-2.1 5.3l1-.4m5.4-2.4l1-.4" /></svg>',
  ...
  creditCard: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="1" y="4" width="22" height="16" rx="2" ry="2" /><line x1="1" y1="10" x2="23" y2="10" /></svg>',
  receipt: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1-2-1z" /><path d="M8 10h8M8 14h4" /></svg>',
  chart: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" /></svg>',
  message: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" /></svg>',
  support: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10" /><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>',
  briefcase: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M5 2h10a2 2 0 012 2v16a2 2 0 01-2 2H5a2 2 0 01-2-2V4a2 2 0 012-2z" /><path d="M17 8l4 2v4l-4 2" /><circle cx="13" cy="12" r="1" fill="currentColor" /></svg>',
  clock: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>',
  calendarCheck: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /><path d="M9 16l2 2 4-4" /></svg>',
  wallet: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M20 12V8H6a2 2 0 01-2-2c0-1.1.9-2 2-2h12v4" /><path d="M4 6v12a2 2 0 002 2h14v-4" /><path d="M18 12a2 2 0 00-2 2v4h4v-4a2 2 0 00-2-2z" /></svg>',
  fileText: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>',
  table: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><line x1="3" y1="9" x2="21" y2="9" /><line x1="3" y1="15" x2="21" y2="15" /><line x1="9" y1="3" x2="9" y2="21" /><line x1="15" y1="3" x2="15" y2="21" /></svg>',
  ...
  checkCircle: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M22 11.08V12a10 10 0 11-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>',
  barChart: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="20" x2="12" y2="10" /><line x1="18" y1="20" x2="18" y2="4" /><line x1="6" y1="20" x2="6" y2="16" /></svg>',
  pieChart: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M21.21 15.89A10 10 0 118 2.83" /><path d="M22 12A10 10 0 0012 2v10z" /></svg>',
  ...
  myLeaves: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /><path d="M12 12v4m0 0l-2-2m2 2l2-2" /></svg>',
...
const menuItems = computed(() => {
  ...
  if (role === 'super_admin') {
    ...
    items.push({ path: '/commission-management', name: 'Komisyon Yönetimi', icon: icons.commission })
