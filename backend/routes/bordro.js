 ...
 * Rol: bayi_admin
 */
router.post('/upload', auth, requireRole('super_admin', 'bayi_admin'), upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    if (existingBordros.length > 0) {
      ...
      if (statusCounts.approved > 0 && statusCounts.rejected === 0 && statusCounts.pending === 0 && statusCounts.company_approved === 0) {
        // Tüm bordrolar çalışan tarafından onaylanmış, güncellenecek bir şey yok
        if (req.file.path) fs.unlinkSync(req.file.path);
        return errorResponse(res, {
          message: 'Bu dönemdeki tüm bordrolar çalışanlar tarafından onaylanmış. Güncellenecek bordro bulunmuyor.',
          code: 'ALL_APPROVED'
        });
      }
    ...
    bordroUpload.status = result.errors.length > 0 && result.successCount > 0 ? 'partial' :
 ...
 * Rol: bayi_admin
 */
router.post('/uploads/:id/notify', auth, requireRole('super_admin', 'bayi_admin'), async (req, res) => {
  try {
    const upload = await BordroUpload.findById(req.params.id)
      .populate('company');
    ...
    for (const admin of companyAdmins) {
      try {
        ...
        await notificationService.send({
          ...
          data: { uploadId: upload._id, year: upload.year, month: upload.month, count: pendingCount },
 ...
 */
router.get('/stats', auth, requireRole('super_admin', 'bayi_admin', 'company_admin'), async (req, res) => {
  try {
    let companyFilter = {};
 ...
 */
router.get('/company-list', auth, requireRole('super_admin', 'bayi_admin', 'company_admin'), async (req, res) => {
  try {
    let query = {};
 ...
 * Rol: bayi_admin
 */
router.get('/rejections', auth, requireRole('super_admin', 'bayi_admin', 'company_admin'), async (req, res) => {
  try {
 ...
 * Rol: company_admin
 */
router.get('/pending-approval', auth, requireRole('company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
    const userCompanyId = req.user.company?._id || req.user.company;
 ...
 * Rol: company_admin
 */
router.post('/:id/company-approve', auth, requireRole('company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
    const { note } = req.body;
 ...
 * Rol: company_admin
 */
router.post('/bulk-approve', auth, requireRole('company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
 ...
 * Rol: company_admin
 */
router.post('/:id/company-reject', auth, requireRole('company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
...
router.get('/my-bordros', auth, async (req, res) => {
  try {
    ...
    if (!req.user.employee) {
      return errorResponse(res, {
        message: 'Hesabınız çalışan kaydına bağlı değil. Lütfen sistem yöneticisi ile iletişime geçin.'
      }, 403);
    }
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const companyAdminRoles = ['company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'];
 ...
 */
router.delete('/:id', auth, requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
    const bordro = await Bordro.findById(req.params.id)
      .populate('company');
 ...
 * Çalışan için onay kodu iste (email ile gönderilir)
 * Rol: employee
 */
router.post('/:id/request-approval-code', auth, async (req, res) => {
  try {
    ...
    // Rate limiting - 1 dakikada bir kod istenebilir
    if (bordro.employeeApprovalCodeLastRequest) {
      const timeDiff = Date.now() - bordro.employeeApprovalCodeLastRequest.getTime();
      const waitTime = 60000 - timeDiff; // 1 dakika
      if (waitTime > 0) {
        return errorResponse(res, {
          message: `Yeni kod istemek için ${Math.ceil(waitTime / 1000)} saniye bekleyin`
        });
      }
    }
    ...
    // Kodu kaydet (15 dakika geçerli)
    bordro.employeeApprovalCode = codeHash;
    bordro.employeeApprovalCodeExpires = new Date(Date.now() + 15 * 60 * 1000);
    ...
    try {
      const emailService = require('../services/emailService');
      await emailService.sendBordroApprovalCodeEmail(
        bordro.employee, // employee object with email, firstName, lastName
        code,
        bordro.employeeApprovalCodeExpires
      );
    } catch (emailError) {
      console.error('Onay kodu emaili gönderilemedi:', emailError);
      return serverError(res, emailError, 'Onay kodu gönderilemedi');
    }

    return successResponse(res, {
      message: 'Onay kodu email adresinize gönderildi',
      email: bordro.employee.email.replace(/(.{2})(.*)(@.*)/, '$1***$3'), // Maske
      expiresAt: bordro.employeeApprovalCodeExpires
    });
  } catch (error) {
    return serverError(res, error);
  }
 ...
 * Çalışan bordroyu onaylar (kod doğrulama ile)
 * Rol: employee
 */
router.post('/:id/employee-approve', auth, async (req, res) => {
  try {
    ...
    if (bordro.employeeApprovalCodeAttempts >= 3) {
      return errorResponse(res, {
    ...
    // Kod süresi kontrolü
    if (!bordro.employeeApprovalCodeExpires || bordro.employeeApprovalCodeExpires < new Date()) {
      return errorResponse(res, { message: 'Onay kodunun süresi dolmuş. Lütfen yeni kod isteyin.' });
    }
    ...
    if (codeHash !== bordro.employeeApprovalCode) {
      bordro.employeeApprovalCodeAttempts += 1;
      await bordro.save();

      const remaining = 3 - bordro.employeeApprovalCodeAttempts;
      return errorResponse(res, {
        message: `Hatalı kod. ${remaining} deneme hakkınız kaldı.`
      });
    }
    ...
    bordro.employeeApprovedIp = req.ip || req.connection.remoteAddress;
    ...
    try {
      await notificationService.send({
        ...
        body: `${bordroService.getMonthName(bordro.month)} ${bordro.year} dönemi bordronuzu başarıyla onayladınız.`,
    ...
    return successResponse(res, {
      message: 'Bordronuz başarıyla onaylandı',
...
router.get('/:id/download-pdf', auth, async (req, res) => {
  try {
    const bordro = await Bordro.findById(req.params.id)
      .populate('employee', 'firstName lastName tcKimlik position hireDate lastHireDate employeeNumber workplace')
...
router.get('/:id/verify-timestamp', auth, async (req, res) => {
  try {
    ...
    // Doğrula
    const verifyResult = await timestampService.verifyTimestamp(pdfBuffer, bordro.timestampToken, bordro);
 ...
 */
router.get('/admin/timestamp-health', auth, requireRole('super_admin', 'bayi_admin'), async (req, res) => {
  try {
 ...
 * Rol: company_admin, bayi_admin, super_admin
 */
router.post('/:id/revert-to-pending', auth, requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
    const bordro = await Bordro.findById(req.params.id)
      .populate('employee', 'firstName lastName')
      .populate('company');
 ...
 * Rol: company_admin, bayi_admin, super_admin
 */
router.post('/bulk-revert', auth, requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
    ...
    const companyApprovedCount = bordrosToRevert.filter(b => b.status === 'company_approved').length;
 ...
 * Rol: company_admin, bayi_admin, super_admin
 */
router.post('/bulk-delete', auth, requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik', 'SIRKET_ADMIN', 'IK_OPERASYON'), async (req, res) => {
  try {
    const { bordroIds } = req.body;
    ...
    if (bordrosToDelete.length === 0) {
      return errorResponse(res, { message: 'Silinebilecek bordro bulunamadı. Sadece "Beklemede" durumundaki bordrolar silinebilir.' });
    }

    // Upload istatistiklerini güncelle
    const uploadIds = [...new Set(bordrosToDelete.filter(b => b.upload).map(b => b.upload.toString()))];
    for (const uploadId of uploadIds) {
      const deleteCountForUpload = bordrosToDelete.filter(b => b.upload && b.upload.toString() === uploadId).length;
