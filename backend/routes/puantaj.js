...
const LUCA_PUANTAJ_CODES = [
  ...
  },
  {
    ...
    color: '#006633',
    textColor: '#FFFFFF',
    autoAssignType: 'day_overtime',
    sortOrder: 10,
  },
  {
    code: 'K',
    name: 'Yarım Gün Resmi Tatil',
    color: '#FF99CC',
    textColor: '#000000',
    autoAssignType: 'half_public',
    sortOrder: 11,
  },
  {
    code: 'C',
    name: 'Yarım Gün Hafta Tatili',
    color: '#99CCFF',
    textColor: '#000000',
    autoAssignType: 'half_weekend',
    sortOrder: 12,
  },
];
...
router.get('/templates', auth, async (req, res) => {
  try {
    ...
    if (req.user.company) {
      query.$or = [{ company: null }, { company: req.user.company }];
    }
...
router.get('/templates/:id', auth, async (req, res) => {
  try {
    const template = await PuantajTemplate.findById(req.params.id).populate('company', 'name');
...
router.get('/templates/:templateId/codes', auth, async (req, res) => {
  try {
    const codes = await PuantajCode.find({ template: req.params.templateId }).sort({
      sortOrder: 1,
    });
    res.json(codes);
  } catch (error) {
...
router.put(
  ...
  async (req, res) => {
    try {
      const code = await PuantajCode.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
router.post(
  '/employee/:employeeId/:year/:month/regenerate',
  auth,
...
router.post(
  '/company/:companyId/:year/:month/regenerate',
  auth,
  requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik'),
  async (req, res) => {
    try {
      ...
      for (const employee of employees) {
        ...
        // Yeniden oluştur
        const newPuantaj = await generatePuantaj(
          employee,
          template,
          parseInt(year),
          parseInt(month)
        ...
        if (preserveManualOverrides && Object.keys(manualOverrides).length > 0) {
          for (const [dateKey, override] of Object.entries(manualOverrides)) {
            const dayIndex = newPuantaj.days.findIndex(
              d => new Date(d.date).getDate() === parseInt(dateKey)
            );
            if (dayIndex !== -1) {
...
router.put(
  '/employee/:employeeId/:year/:month/day/:day',
  auth,
  requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik'),
  async (req, res) => {
    try {
      ...
      const dayIndex = puantaj.days.findIndex(d => new Date(d.date).getDate() === parseInt(day));
...
router.put(
  '/employee/:employeeId/:year/:month/manual-overtime',
  auth,
  requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik'),
  async (req, res) => {
    try {
...
// SGK günü güncelle
router.put(
  '/employee/:employeeId/:year/:month/sgk-gun',
  auth,
  requireRole('super_admin', 'bayi_admin', 'company_admin', 'resmi_muhasebe_ik'),
  async (req, res) => {
    try {
      const { employeeId, year, month } = req.params;
      const { sgkGun } = req.body;

      if (sgkGun === undefined || sgkGun === null || sgkGun < 0) {
        return errorResponse(res, { message: 'Geçerli bir SGK günü giriniz' });
      }
...
router.get('/company/:companyId/:year/:month', auth, async (req, res) => {
  try {
    ...
    const employees = await Employee.find({
      ...
      .select(
        'firstName lastName employeeNumber personelNumarasi department hireDate isRetired tcKimlik'
      )
      .populate('department', 'name');
...
router.get('/reports/summary', auth, async (req, res) => {
  try {
    ...
    const totalSummary = {
      ...
      totalOvertime: reportData.reduce(
        (sum, r) => sum + r.dayOvertimeHours + r.nightOvertimeHours,
        0
      ),
      totalLeaveDays: reportData.reduce(
        (sum, r) => sum + r.annualLeaveDays + r.sickLeaveDays + r.otherLeaveDays,
        0
      ),
...
async function getEffectiveWeekendDays(employee) {
  ...
  if (
    ...
    company.leaveSettings.weekendDays &&
    company.leaveSettings.weekendDays.length > 0
  ) {
...
async function generatePuantaj(employee, template, year, month) {
  ...
  const codeMap = {};
  codes.forEach(c => {
    codeMap[c.autoAssignType] = c.code;
  });
  ...
  const holidays = holidayCalendar
    ? holidayCalendar.holidays.map(h => h.toISOString().split('T')[0])
    : [];
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      if (leave.companyLeaveType) {
        ...
        if (leaveTypeName.includes('yıllık') || leaveTypeName.includes('yillik')) {
          ...
          leaveTypeName.includes('hastalik')
        ) {
...
function recalculateSummary(puantaj) {
  ...
  for (const day of puantaj.days) {
    switch (day.code) {
      ...
      case 'T':
        ...
        break;
      case 'S':
        summary.annualLeaveDays++;
        break;
      case 'R':
        summary.sickLeaveDays++;
        break;
      case 'İ':
        summary.otherLeaveDays++;
        break;
      case 'E':
        summary.absentDays++;
        break;
  ...
  // SGK günü henüz set edilmemişse, çalışma gününe eşitle
  if (puantaj.sgkGun === null || puantaj.sgkGun === undefined) {
    puantaj.sgkGun = summary.normalDays;
  }
}
